...
export function MusicPlayer({ track, onClose, queue = [] }: MusicPlayerProps) {
  ...
  const [playQueue] = useState<Song[]>(queue)
  ...
  useEffect(() => {
    ...
    if (track.song_id) {
      userApi.addToRecentlyPlayed(track.song_id).catch(() => {})
    }
  ...
  }, [track, volume, currentTrackIndex, playQueue])

  // Check if track is favorite
  useEffect(() => {
    if (track.song_id) {
      userApi.isFavorite(track.song_id).then(setIsFavorite).catch(() => {})
    }
  }, [track])
  ...
  const handleNext = () => {
    if (currentTrackIndex < playQueue.length - 1) {
      ...
      // In a real app, you'd update the parent component to change the track
      toast(`Playing next: ${playQueue[nextIndex].title}`, {
        icon: '🎵',
      })
    }
  }
  ...
  const handlePrevious = () => {
    if (currentTrackIndex > 0) {
      ...
      // In a real app, you'd update the parent component to change the track
      toast(`Playing previous: ${playQueue[prevIndex].title}`, {
        icon: '🎵',
      })
    }
  }
  ...
  const toggleFavorite = async () => {
    ...
    try {
      if (isFavorite) {
        await userApi.removeFavorite(track.song_id)
        toast.success('Removed from favorites')
      } else {
        await userApi.addFavorite(track.song_id)
      ...
      <div className="container mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center space-x-6">
          <button 
            ...
            disabled={currentTrackIndex === 0 && playQueue.length === 0}
